...
import { BadgeCheck, Lock, MessageSquare } from "lucide-react";

const POINTS = [
  { icon: BadgeCheck, text: "Annonces et bailleurs vérifiés" },
  { icon: Lock, text: "Acompte protégé jusqu'à la visite" },
  { icon: MessageSquare, text: "Échanges gardés dans l'application" },
];
    ...
    <main className="app-ambient flex min-h-dvh w-full items-center justify-center lg:py-8">
      <div className="relative flex h-dvh w-full flex-col overflow-hidden bg-card lg:h-[860px] lg:w-[392px] lg:rounded-[2.6rem] lg:shadow-soft">
        <div className="relative h-72 shrink-0">
          <Image src="/img/listings/demo-1.jpg" alt="" fill className="object-cover" priority sizes="392px" />
        ...
        <div className="flex flex-1 flex-col justify-between p-6">
          <ul className="space-y-4">
            {POINTS.map(({ icon: Icon, text }) => (
              <li key={text} className="flex items-center gap-3">
                <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-brand-50 text-accent">
          ...
          <div className="space-y-3">
            <Link
              href="/explore"
              className="flex items-center justify-center rounded-xl bg-primary py-3.5 text-sm font-bold text-primary-foreground shadow-btn"
            >
              Commencer
            </Link>
