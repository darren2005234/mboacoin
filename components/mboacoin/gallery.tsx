...
export function Gallery({ images, alt = "" }: GalleryProps) {
  ...
  return (
    <>
      <div className="relative h-60 w-full shrink-0 overflow-hidden bg-secondary">
        ...
        >
          {images.map((src, i) => (
            ...
            >
              <Image src={src} alt={alt} fill className="object-cover" sizes="448px" priority={i === 0} />
            </button>
          ))}
        </div>
        ...
        {images.length > 1 && (
          <div className="pointer-events-none absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5">
            {images.map((_, i) => (
              <span key={i} className={i === index ? "size-2 rounded-full bg-white" : "size-2 rounded-full bg-white/50"} />
            ))}
          </div>
        )}
      ...
      {zoomed && (
        <Lightbox images={images} startIndex={index} onClose={() => setZoomed(false)} />
      )}
    </>
  );
}